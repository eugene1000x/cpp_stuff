...
//changed 738: register at -> register long at
#define zaddmulp(_a, _b, _d, _t) \
{ \
	register at = *(_a) + *(_t); \
...
//changed 748: register at -> register long at
#define zaddmulpsq(_a, _b, _t) \
{ \
	register at = *(_a); \
...
#else
	struct tms tbuf; times(&tbuf); return(1.0*tbuf.tms_utime/CLOCKS_PER_SEC);
#endif
}
...
{
	static char *b = 0;
	static bl = 0;//changed: static bl -> static long bl
	STATIC verylong aa = 0;
	register long i;
	register long cnt = 0;
	register long mb;
...
stage2:
	...
	for (; p>1; p--) {
		register e;//changed: register e -> register long e
